...
namespace dt {
	...
	template<typename T>
	//深拷贝构造函数，（浅拷贝容易出问题，_vertices.push_back）
	Triangle<T>::Triangle(const VertexType &v1, const VertexType &v2, const VertexType &v3) 
		//a(&v1), b(&v2), c(&v3), isBad(false)
	{
		
		isBad = false;
		a = new VertexType(v1.x, v1.y, v1.index);
		b = new VertexType(v2.x, v2.y, v2.index);
		c = new VertexType(v3.x, v3.y, v3.index);
	...
	/*template<typename T>
	bool
		Triangle<T>::containsVertex() const
	{
	}*/
	/*template<typename T>
	void Triangle<T>::setTriangle(const VertexType &v1, const VertexType &v2, const VertexType &v3) {
		a = &v1;
		b = &v2;
		c = &v3;
	}
*/
	template<typename T>
	bool Triangle<T>::circumCircleContains(const VertexType &v) const
	{
		const T ab = a->norm2();
		const T cd = b->norm2();
		const T ef = c->norm2();
	...
	template<typename T>
	void Triangle<T>::shallowCopyTriangle(const Triangle &other) {
		a = other.a;
		b = other.b;
		c = other.c;
		isBad = other.isBad;
